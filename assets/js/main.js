class uploader {
    constructor(options) {
        ...
        this.fileElem = options.fileElem;
        this.showElem = (options.showElem) ? options.showElem : undefined;
        this.uploadPath = options.uploadPath;
        this.chunkSize = (options.chunkSize == undefined || options.chunkSize <= 0) ? (512 * 1024) : options.chunkSize;
            ...
            if (self.fileElem.files.length > 0) {
                ...
                for (counter = 0; counter < length; counter++) {
                    ...
                    if (!self.checkExtension(file)) {
                        ...
                        self.preSetState(0, file, request, null, 0, false, false, self.counter);

                        UI = self.createUI(self.counter);
                        self.updateStateUI(self.counter, UI);

                        self.setUploadHandlers(self.counter);
                ...
                if (self.states[stateNumber].request.readyState == 4) {
                    if (self.states[stateNumber].request.status == 200) {
                        ...
                        if (self.states[stateNumber].chunkPointer < self.states[stateNumber].file.size) {
                            ...
                            console.log('uploaded', self.states[stateNumber].file.name);
                        }
                    } else {
                ...
                if (!(self.states[stateNumber].file instanceof File)) {
                    ...
                    file.addEventListener('change', function () {
                        if (file.files[0].size == self.states[stateNumber].file.size) {
                            console.log('start to resume upload');
                            ...
                            alert('selected file must be the same as the uncompleted file.');
                        }
                    }, false);
                    file.click();
                } else {
...
(new uploader({
    ...
    requestDelay:100,
    chunkSize:(1024),
    validExtensions:["jpg", "jpeg", "bmp", "gif", "png","mp3","mp4","avi","zip","docx","xlsx"],
    showElem:document.querySelector('#uploadTable tbody')
}).init().loadFromDB('./uploader/upload.php'));